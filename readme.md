...
关于单元测试的建议:    
我比较推荐的单元测试方式是只对Executor的单元测试，Executor层是业务的编排逻辑，对Executor测试可直接覆盖业务，当然Executor层也引用了资源，我的建议是对资源做mock处理，可参考demo下的单元测试代码。若需要对资源层测试可以提供一套资源层测试的代码，我觉得这个比较简单，这里只测试资源操作不关心数据业务，也就是说对于单元测试我们将分开对业务的测试与数据的测试。
当然也可以参考COLA MOCK的方式来实现单元测试业务。
